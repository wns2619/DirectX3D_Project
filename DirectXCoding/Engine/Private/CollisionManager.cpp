...
void CollisionManager::LateTick(const _float& timeDelta)
{
	...
	for (uint32 iRow = 0; iRow < static_cast<uint32>(LAYER_TAG::LAYER_END); ++iRow)
		for (uint32 iCol = iRow; iCol < static_cast<uint32>(LAYER_TAG::LAYER_END); ++iCol)
			if (_arrCheckAssist[iRow] & (1 << iCol))
				CheckAssistCollision(static_cast<LAYER_TAG>(iRow), static_cast<LAYER_TAG>(iCol));
}
...
void CollisionManager::CheckAssistCollision(LAYER_TAG eLayerTagLeft, LAYER_TAG eLayerTagRight)
{
	GameInstance* pGameInstance = GET_INSTANCE(GameInstance);
...
void CollisionManager::CheckAssisGroup(LAYER_TAG eLeft, LAYER_TAG eRight)
{
	uint32 iRow = static_cast<uint32>(eLeft);
	uint32 iCol = static_cast<uint32>(eRight);
